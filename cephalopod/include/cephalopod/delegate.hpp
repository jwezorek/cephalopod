...
* Specialy modified by use in the Acclimate Engine
*
...
*
* Modified by Julian Watzinger 10/12/13
*
...
//
// 10-Dec-13 1.6 Julian Watzinger
// * Using varidic templates for unlimited numbers of delegate argument numbers,
//   also reduced amount of code
...
namespace detail {
    ...
    public:
        ...
        inline void binAclmfunc(X *pthis, XMemFunc function_to_bind ) {
        ...
        // VC6 has problems if we just overload 'binAclmfunc', so we give it a different name.
        template < class X, class XMemFunc>
        ...
        template < class X, class XMemFunc>
        inline void binAclmfunc(const X *pthis, XMemFunc function_to_bind) {
        ...
        inline void bindstaticfunc(DerivedClass *pParent, ParentInvokerSig static_function_invoker,
            StaticFuncPtr function_to_bind ) {
                if (function_to_bind==0) { // cope with assignment to 0
                    ...
                    binAclmfunc(pParent, static_function_invoker);
                }
...
#pragma warning( disable: 4251 )
	typedef detail::DefaultVoid RetType;
    typedef typename detail::DefaultVoidToVoid<RetType>::type DesiredRetType;
    typedef DesiredRetType (*StaticFunctionPtr)(Args... args);
    typedef RetType (*UnvoidStaticFunctionPtr)(Args... args);
    typedef RetType (detail::GenericClass::*GenericMemFn)(Args... args);
...
public:
    ...
    Delegate(Y *pthis, DesiredRetType (X::* function_to_bind)(Args... args) ) {
        m_Closure.binAclmfunc(detail::implicit_cast<X*>(pthis), function_to_bind); }
    template < class X, class Y >
    inline void Bind(Y *pthis, DesiredRetType (X::* function_to_bind)(Args... args)) {
        m_Closure.binAclmfunc(detail::implicit_cast<X*>(pthis), function_to_bind);	}
    // Binding to const member functions.
    template < class X, class Y >
    Delegate(const Y *pthis, DesiredRetType (X::* function_to_bind)(Args... args) const) {
...
Delegate<Args...> MakeDelegate(Y* x, RetType (X::*func)(Args... args)) {
    return Delegate<Args...>(x, func);
}

template <class X, class Y, class RetType, typename... Args>
Delegate<Args...> MakeDelegate(Y* x, RetType (X::*func)(Args... args) const) {
    return Delegate<Args...>(x, func);
}
