...
namespace ceph
{
	...
	public:
		...
		bool isInSceneTransition() const override;
		SceneTransition& getSceneTransition() override;
		void setScene(const std::shared_ptr<Scene>& scene, const std::shared_ptr<SceneTransition> transition) override;
		void clearTransition() override;
	};
};